...
Cond.to.Sal <- function(cond, temp, ref_cond = 42.9){
  ...
  var1 <- cond[i]/ref_cond

  # rt
  var2 <- 0.6766097 + (0.0200564 * temp[i]) + (0.0001104259 * temp[i]^2) + ((-6.9698 * 10^-7) * temp[i]^3) + ((1.0031 * 10^-9) * temp[i]^4)

  # Rt
  var3 <- var1/var2

  # dS
  var4 <- ((temp[i] - 15) / (1 + (0.0162 * (temp[i] - 15)))) * (0.0005 + ((-0.0056) * var3^0.5) + ((-0.0066) * var3) + ((-0.0375) * var3^1.5) + ((0.0636) * var3^2) + ((-0.0144) * var3^2.5))

  Sal[i] <- 0.008 + ((-0.1692) * var3^0.5) + (25.3851 * var3) + (14.0941 * var3^1.5) + ((-7.0261) * var3^2) + (2.7081 * var3^2.5) + var4
  }
